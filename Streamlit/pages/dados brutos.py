...
with st.sidebar.expander('Categoria do produto'):
    categoria = st.multiselect(
        'Selecione as categorias', dados['Categoria do Produto'].unique(), dados['Categoria do Produto'].unique())

with st.sidebar.expander('Preço do produto'):
    preco = st.slider('Selecione o preço', 0, 5000, (0, 5000))

with st.sidebar.expander('Frete'):
    frete = st.slider('Valor do frete', 0,
                      250, (0, 250))
...
with st.sidebar.expander('Local da compra'):
    local_compra = st.multiselect(
        'Selecione o local da compra', dados['Local da compra'].unique(), dados['Local da compra'].unique())

with st.sidebar.expander('Avaliação da compra'):
    avaliacao = st.slider('Avaliação da compra', 1, 5, (1, 5))
...
with st.sidebar.expander('Quantidade de parcelas'):
    parcelas = st.slider('Quantidade de parcelas', 1, 24, (1, 24))
...
with coluna2:
    st.download_button('Fazer o download da tabela em csv', data=convert_csv(
